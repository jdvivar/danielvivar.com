<template>
  <section id="future">
    <h1>what I'm looking for</h1>
    <div class="section-description">
      <p>
        My future company should be a place where <b>learning</b> and sharing knowledge is <b>actively encouraged</b>.
      </p>
      <p>
        I feel specially motivated in projects where we can collaborate into creating <b>delightful user experiences</b> for great apps.
      </p>
      <p>
        Other features that thrive me are an <b>open-minded culture</b>, flexibility, a healthy environment, a focused team and companies where technology is the heart.
      </p>
    </div>
  </section>
</template>

<script>
export default {
  name: 'Future'
}
</script>
